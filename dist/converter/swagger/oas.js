...
const blueprint_js_1 = require("../../core/blueprint.js");
/**
 * ConverterOAS
 */
class ConverterOAS {
    ...
    convert(oas, options) {
        ...
        blueprint.setApplications(new Application(options.appsName, options.packages, options.framework, (0, blueprint_js_1.getFront)(this.model), utils.getServer(this.model)));
        blueprint.setEntities(this.getComponents(oas));
        if (oas.securityDefinitions) {
            // Swageer 2.x
            blueprint.setSecurity(this.getSecurityDefinitions(oas.securityDefinitions));
        }
    ...
    getSecurityDefinitions(securityDefinitions, options) {
        ...
        Object.entries(securityDefinitions).forEach((e) => {
            ...
            if (e[1].in)
                sec.in = e[1].in;
            if (e[1].authorizationUrl)
                sec.authorizationUrl = e[1].authorizationUrl;
            if (e[1].flow)
                sec.flow = e[1].flow;
            if (e[1].scopes) {
                sec.scopes = [];
    ...
    getComponents(components, options) {
        Object.entries(components).forEach((e) => {
            ...
            if (e[1].properties) {
                entity.setProperties(this.getProperties(e[1].properties, entity.name, entity.modelOnly));
                this.entities.push(entity);
            }
        });
        return this.entities;
    ...
    getDefinitions(definitions, options) {
        Object.entries(definitions).forEach((e) => {
            ...
            if (e[1].properties) {
                entity.properties = this.getProperties(e[1].properties, entity.name, entity.modelOnly);
                this.entities.push(entity);
            }
        });
        return this.entities;
        ...
        Object.entries(properties).forEach((e) => {
            ...
            if (e[1].type) {
                ...
                if (e[1].type === 'object' && e[1].properties) {
                    prop.ref = this.objectBuilder(e[1].properties, prop.name, entityName, modelOnly);
    ...
    getOperations(options) {
        ...
        Object.entries(this.oas.paths).forEach((e) => {
            ...
            Object.entries(e[1]).forEach((m) => {
                ...
                // Parameters
                const param = this.getParameters(m[1].parameters, operation.path, options);
    ...
    getResponses(responses, method, options) {
        ...
        Object.entries(responses).forEach((r) => {
            ...
            if (r[1].content) {
                ...
                response.content = this.getContentsOAS(r[1].content, method, options);
            }
            else {
                // Swagger 2.x
                if (r[1].schema) {
                    response.content = this.getProduces(r[1].schema, method, options);
                }
            }
            ...
            data.forEach((r) => {
                ...
                if (r.schema && r.schema.type) {
                    ...
                    schema:
                      ...
                      minItems: 1
                    */
                    param.type = utils.convertTypeByFramework(r.schema, '', this.framework);
                    if (r.schema.minimum)
                ...
                if (c[1].schema.type) {
                    ...
                    if (c[1].schema.type === 'object') {
                        ...
                        if (options.propertiesAsEntity) {
                            const respEntityName = objectBuilder(c[1].schema.properties, response.code, 'Resp', this.entities, true);
                            content.ref = respEntityName;
                        }
                        else {
                            content.properties = this.getProperties(c[1].schema.properties, 'Cont', this.entities, true);
                        }
                    }
                    /*
                      {
                    ...
                    */
                    if (c[1].schema.type === 'array') {
                        ...
                        content.type = utils.convertTypeByFramework(c[1].schema, ref, this.framework);
                    }
                }
                /*
                  {
