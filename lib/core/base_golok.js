...
export default class BaseGolok {
        ...
        .action((modelPath, options) => {
          ...
          try {
            ...
            if (modelPath === 'exampleModel' ) {
              ...
              model = utils.transpile(yaml, {isExample:true, exampleDir:this.__dirname+'/../../'});
            } else {
            ...
            this.manifest = this.templateManifest(options.template?
                options.template + this.MANIFEST : this.__dirname +'/../generator/'+
  ...
  */
  ...
  parseEntities(model) {
    model.entities.forEach((el) => {
      ...
      el.properties.forEach((prop, i) => {
        ...
        el.properties[i] = this.typeParsing(el.properties[i], relationship, model);
      });
   ...
   * @return {String} value
   */
  typeParsing(prop, relationship, model) {
    // Check by framework used
    const framework = model.applications.frontend.framework;
    if (framework == 'flutter') {
      prop = utils.convertTypeDart(prop, prop.ref? prop.ref.name:prop.type, relationship);
    }
