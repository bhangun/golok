...
import {typeCheck, camelize, copyFile, convertTypeDart,
  print, makeDir, camelToTitle, camelToSnake, longtime, json2js,
  renderEjsFile, checkFileExt, parseYaml, getCurrentDirname,
} from './utils.js';
        ...
        .action((modelPath, options) => {
          ...
          try {
            ...
            if (modelPath === 'exampleModel' ) {
              print('You\ aren\'t defined the model, '+
              'so we will used the example', 'yellow');
              model = parseYaml(this.MODEL_EXAMPLE);
            } else {
              const ext = checkFileExt(modelPath);
              if ( ext === '.yaml') {
                model = parseYaml(modelPath);
              } else if ( ext === '.json') {
                model = json2js(modelPath);
              }
            }
            ...
            if(framework){
              ...
              this.framework = this.config.model.applications.frontend.framework;
            }
...
//const templateDir = obj.frontendTemplatePath(config);
