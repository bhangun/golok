...
export default class BaseGolok {
  ...
  */
  ...
  console.log(path+_framework + template)
  ...
  parseEntities(model) {
    model.entities.forEach((el) => {
      ...
      el.nameClass = utils.camelToTitle(el.name);
      el.nameField = utils.camelize(el.name);
      el.nameFile = utils.camelToSnake(el.name);
  ...
  typeParsing(prop, model) {
    ...
    if (model.applications.frontend.framework == 'flutter') {
      prop = utils.convertTypeDart(prop, prop.ref? prop.ref.name:'');
    }
