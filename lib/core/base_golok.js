...
export default class BaseGolok {
    ...
    this.program.command('create')
        ...
        .action((modelPath, options) => {
          ...
          try {
            ...
            if (options.jdl) {
              transpileJDL(model, this.config.options.output, this.__dirname);
            }

            if (options.jdljson) {
              transpileJDLJson(model, this.config.options.output);
            }
  ...
  renderEntities(templateDir, entities, manifest, toDir, model, options) {
    ...
    manifest.templates[0].templates[1].items.forEach((item) => {
  ...
  */
  ...
  parseEntities(model) {
    ...
    model.entities.forEach((el) => {
      ...
      el.nameTitleCase = utils.camelToTitle(el.name);
      el.nameCamelCase = utils.camelize(el.name);
      el.nameSnakeCase = utils.camelToSnake(el.name);
