import {
    typeCheck, convertTypeDart, camelToTitle,
    convertTypeJava, camelToTitleWithSpace, print
} from './utils.js';
            ...
            Object.entries(rawProperties).forEach((prop) => {
                ...
                if (_param !== null && typeCheck(_param) === 'string') {
                    ...
                    if (value.length > 0) {
                        ...
                        const localeDoc = prop[1].match(/{(.*?)}/);
                        if (localeDoc) {
                            props.docID = localeDoc[1].match(/id:(.*),./)[1];
                            props.docEN = localeDoc[1].match(/.,.en:(.*)/)[1];
                        }
                        if (!props.docID) {
