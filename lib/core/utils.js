...
function mergeIncludes(yaml, options) {
  if(yaml.includes){
    ...
    yaml.includes.forEach(el => {
      var _path = options.isExample? options.exampleDir+el.file : el.file;
      ...
      // Merge entities
      if(el.entities){
        onlyEntities = el.entities.trim().split(',');
      ...
      // Merge enums
      if(el.enums){
        onlyEnums = el.enums.trim().split(',');
        console.log(onlyEnums)
      }else{
        if(_yaml.enums){
          if(!yaml.enums){
            yaml.enums = [];
...
console.log(yaml.entities)
console.log(yaml.enums)
  return yaml;
}
