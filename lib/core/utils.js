...
export {
    typeCheck, camelize, print, write, copyFile, cleanOtherChar,
    makeDir, renderEjsFile, getLastModifiedDate, Framework, writeFile,
    titleCase, camelToSnake, longtime, getCurrentDirname,
    camelToTitle, capitalize, checkFileExt, yml2js, 
    json2js, parseYaml, getLastWord,/* transpile, */
    removeWhitespace, splitString, getDirectory, camelToTitleWithSpace,
    extractLocale, getMimeType, streamToString, streamToFile, parseYamlString,
...
function cleanOtherChar(text) {
    if (text) {
        return text.replace(/[-+/\|$%()^&*@#!<>{}]\w+/g,
            (letter) => `${letter.substring(1, 1).toUpperCase() +
                letter.substring(2)}`).replace(/[-+/\|$%()^&*@#!<>{}]+/g, '');
    } else return text;
}
