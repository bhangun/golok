...
import {typeCheck, camelize, copyFile, convertTypeDart,
  print, makeDir, titleCase, renderEjsFile} from './utils.js';
...
export default class BaseGolok {
  ...
  renderEntities(templateDir, entities, manifest, toDir, context) {
    ...
    manifest.templates[0].templates[0].templates[1].items.forEach((i) => {
      ...
      entities.forEach((e) => {
        ...
        const pathFile = i.to.replace(/{entity}/g, camelize(e.name));
        ...
        new Promise((resolve) => {
          setTimeout(() => {
            renderEjsFile(source, toDir, pathFile, e);
            resolve();
          }, 1000);
        });
      });
    ...
    model.entities.forEach((el) => {
      ...
      el.properties.forEach((prop, i) => {
        ...
        /* if (prop.ref && prop.ref.name) {
          // Check reference type
          // this.checkMatch(model.entities, titleCase(prop.ref.name));
          try {
            const value = this.checUnkMatch(model.entities,
                titleCase(prop.ref.name));
            if (value) {
              throw print('There\'s no reference with this name:'+ value,
                  'red');
            }
          } catch (e) {
          // print('Please check your reference type.', 'magenta');
          }
        } */
  ...
  checkUnMatch(list, value) {
    list.find(function(el) {
      if (value == el.name) {
        throw print('There\'s no reference/ entity with this name: '+
         el.name+'.', 'red');
        return {'value': value};
      }
    });
