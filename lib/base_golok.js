...
import {typeCheck, camelize, copyFile,
  print, makeDir, renderEjsFile} from './utils.js';
...
export default class BaseGolok {
  ...
  generate() {
    ...
    /* const templateDir = this.templateManifest(otherTemplateDir?
        otherTemplateDir + this.MANIFEST:this.DEFAULT_MANIFEST, context);
        ...
        if(context.entities)
            this.renderEntities(templateDir.entity_path,
        context.entities, templateDir.manifest, destination, context);
  ...
  enhanceModel(model) {
    ...
    if (model.entities) {
      model.entities = this.parseProperties(model.entities);
    }
    return model;
  }
