...
import {typeCheck, convertTypeByFramework, getLastWord,
  cleanOtherChar} from '../../core/utils.js';
...
export default class ConverterOAS {
          ...
          */
          if (c[1].schema.type === 'array') {
            ...
            if (c[1].schema.items && c[1].schema.items.$ref) {
              ref = getLastWord(c[1].schema.items.$ref);
            }

            content.type = convertTypeByFramework(c[1].schema,
                ref, this.framework);
          }
        }
  ...
  getProduces(response, method, options) {
    ...
    method.produces.forEach((c) => {
      ...
      if (response.$ref) {
        content.ref = getLastWord(response.$ref);
      }
