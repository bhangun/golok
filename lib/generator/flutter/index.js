...
console.log('----------------------')
        ...
        const templateDir = obj.frontendTemplatePath(config, appConfig.framework,
            appConfig.stateManagement, appConfig.manifest, options);
        const destination = options.output + '/' + appConfig.appsName;

        obj.render(templateDir.app_path, templateDir.entity_path, destination,
            config, this.startTime, appConfig);
