import {
  checkFileExt, printColor, typeCheck, json2js,
  yml2js
} from '../core/utils.js';
import { convertTypeDart } from '../core/type_parser.js';
import Golok from '../core/golok';
import ConverterOAS from './swagger/oas.js';
 ...
 * Converter
 */
export default class Converter extends Golok {
  entities: Array<Entity> = new Array<Entity>;

  EXAMPLE_DIR = __dirname + '/../../example/';
  samplePlainJSON = this.EXAMPLE_DIR + 'contoh.json';
  samplePlainYaml = this.EXAMPLE_DIR + 'contoh.yaml';
  sampleOAS = this.EXAMPLE_DIR + 'oas_strapi.json';
  sampleSwagger = this.EXAMPLE_DIR + 'swagger_2.json';
  inputFile = this.sampleOptions(super.config.options.input,
    super.config.options.example);

  framework = super.config.options.framework;
  /**
   * Construktor
   * @param {String} args
